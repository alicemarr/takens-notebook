...
def plot_forced_system_at_t(tcurr, tsurf0, dt,  ax, y_surf, y_orbit, y_vdp_forced,U, dim_plot =2,len_XX = 100,
                             legend=True, coords = '(x,y)', title = True):
    ...
    if dim_plot==3:
        ax.plot_surface(*np.tensordot(U[:,:dim_plot].T,y_surf_plot, axes = (-1,0)), 
                        color = 'lavender', alpha=0.5, label =  '$\Phi_{(\phi, h,\omega)}(U)$')
    ...
    ax.plot(*np.dot(U[:,:dim_plot].T,np.array(y_orbit)[:, :,tcurr].T), color = 'slateblue', label =  '$\Phi_{(\phi, h,\omega)}(\Gamma)$')

    ax.plot(*np.dot(U[:,:dim_plot].T,y_vdp_forced[0][:,tsurf0+tcurr]), 'X', markersize = 10, label = '$\phi_{\omega_i}%s$'%coords)
